...
These tools **simplify using real objects and test doubles** via dependency injection, so some developers might criticise that the resulting tests are not true unit tests because **class dependencies are not mocked by default**. Mocking is creating objects that **simulate the behaviour of real objects**. Martin Fowler refers to this as the **classic and mockist styles of unit testing**:
